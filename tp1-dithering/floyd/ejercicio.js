  ...
  for (var y = 0; y < image.height; y += 1) {
    for (var x = 0; x < image.width; x += 1) {
      channels.forEach(c => {
        ...
        setPixel(x+1, y  , image, getPixel(x+1, y  , image, c) + quant_error * 7 / 16, c);
        setPixel(x-1, y+1, image, getPixel(x-1, y+1, image, c) + quant_error * 3 / 16, c);
        setPixel(x  , y+1, image, getPixel(x  , y+1, image, c) + quant_error * 5 / 16, c);
        setPixel(x+1, y+1, image, getPixel(x+1, y+1, image, c) + quant_error * 1 / 16, c);
      });
    }
  }
}
